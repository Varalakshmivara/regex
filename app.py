...
def process_pdf_files():
    ...
    if not connect_str:
        logger.error("Azure Storage connection string not found. Set the AZURE_STORAGE_CONNECTION_STRING environment variable.")
        return
    ...
    for pdf_name_data in unextracted_pdf_names:
        pdf_name = pdf_name_data['pdf_name']
        pdf_id = pdf_name_data['pdf_id']
        ...
        save_data_to_table(pdf_files, pdf_name_data)
    return pdf_files


def save_data_to_table(pdf_files, pdf_name_data):
    try:
        ...
        if conn:
            ...
            for data in pdf_files:
                sql_insert = """
                    INSERT INTO [dbo].[Usage] (
                        ...
                        [TransactionSource],
                        [StatusId],
                        [PDFID]   -- Include PDFID column in the insert statement
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """
                ...
                cursor.execute(sql_insert, (
                    ...
                    'OCR',
                    1,
                    pdf_id  # Pass the PDFID from unextracted_pdf_names
                ))
            ...
            cursor.execute("UPDATE pdf_documents SET IsExtracted = 1, Extracted_DateTime = GETDATE() WHERE Pdf_id = ?", (pdf_id,))
        ...
        logger.error(f"Error saving data to table: {str(e)}")
...
def get_unextracted_pdf_names_from_database():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT Pdf_name,pdf_id FROM pdf_documents WHERE Isextracted = 0")
            rows = cursor.fetchall()
            return [{'pdf_name': row[0], 'pdf_id': row[1]} for row in rows]
        except Exception as e:
            logger.error(f"Error fetching unextracted PDF names from database: {str(e)}")
        finally:
            conn.close()
    return []
